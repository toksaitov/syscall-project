...
unsigned long ish_build_pipe_member_array_from_input(
                  char *input,
                  char **pipe_members,
                  unsigned long max_pipe_member_count
              )
{
    unsigned long pipe_member_index = 0;

    for (char *cursor = input;
             cursor && pipe_member_index < max_pipe_member_count;
                 ++pipe_member_index) {
        pipe_members[pipe_member_index] =
            ish_carve_token_in_cstring(cursor, ish_is_pipe, &cursor);
    }
